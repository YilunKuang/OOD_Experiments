...
# encodings =  tokenizer(test['document'], return_tensors='pt', padding="max_length", truncation=True).to(device)
...
for i in range(ids.shape[0]):
    ...
    input_text = test['document'][i]
    input_id =  tokenizer(input_text, return_tensors='pt', padding="max_length", truncation=True).to(device)
    with torch.no_grad():
    ...
    generated_summary = ' '.join(map(str, generated_summary))
    rouge.add(prediction=generated_summary, reference=test['document'][0])
...
# input_txt = torch.from_numpy(ids[0]).to(device)
...
#     input_id = torch.from_numpy(input_id).to(device)
#     with torch.no_grad():
#         result = model.generate(input_ids=input_id, max_length = model.config.max_length)
